...
class cell:
    ...
    def createFrame(self,window):
        ...
        def toggleselectcourse():
            if self.selected:
                self.selected = False
                self.frame.configure(background='systemWindowBackgroundColor')
            else:
                self.selected = True
        ...
        self.Namebuton = tk.Button(master = self.frame, text = self.course.CourseName  , command=toggleselectcourse)
        self.Namebuton.pack( )
